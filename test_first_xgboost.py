...
DATA_DIR = "/media/ubuntu/新加卷1/Kaggle/Featured/Bosch"

# read column names
Num_Cols = pd.read_csv(DATA_DIR + '/train_numeric.csv',
                       nrows=1, dtype=np.float16).columns
Cate_Cols = pd.read_csv(DATA_DIR + '/train_categorical.csv',
                        nrows=1, dtype=str).columns
Date_Cols = pd.read_csv(DATA_DIR + '/train_date.csv',
                        nrows=1, dtype=np.float16).columns
...
Num_dtype_map = {'Id': np.uint16, 'Response':np.uint8}
Cate_dtype_map = {'Id': np.uint16}
Date_dtype_map = {'Id': np.uint16}
...
# file in 'StartTime' column
nrows = 0
tr = pd.read_csv(TRAIN_DATE, nrows=10000, dtype=np.float16)
te = pd.read_csv(TEST_DATE, nrows=10000, dtype=np.float16)
...
print train.columns
print len(train.columns.unique())
# print train.Id.isin(tr.Id)
...
# # calculate [id].diff() up down and reverse
