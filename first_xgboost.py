...
train = pd.read_csv(TRAIN_NUMERIC, usecols=[ID_COLUMN, TARGET_COLUMN], nrows=NROWS, dtype=np.float16)
test = pd.read_csv(TEST_NUMERIC, usecols=[ID_COLUMN], nrows=NROWS, dtype=np.float16)
...
nrows = 0
count = 0
for tr, te in zip(pd.read_csv(TRAIN_DATE, chunksize=CHUNKSIZE, dtype=np.float16), pd.read_csv(TEST_DATE, chunksize=CHUNKSIZE, dtype=np.float16)):
    print count
